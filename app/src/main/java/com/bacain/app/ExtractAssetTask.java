...
public final class ExtractAssetTask {
     ...
     */
    @NonNull
    static Single<File> extractAsync(
            @NonNull final String assetPath, @NonNull final Context context) {
        return Single.<File>create(
                        (emitter) -> {
                            final File outputFile = new File(context.getFilesDir(), assetPath);
                            final DownloadRequest request =
                                    new DownloadRequest.Builder(context)
                            ...
                            job.setProgressListener(
                                    new DownloadJob.ProgressListenerAdapter() {
                                        @Override
